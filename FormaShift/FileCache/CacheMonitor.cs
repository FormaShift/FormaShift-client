...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
    ...
    public void StartMareWatcher(string? marePath)
    {
        ...
        {
            MareWatcher = null;
            Logger.LogWarning("FormaShift file path is not set, cannot start the FSW for Mare.");
            return;
        }
        ...
        Logger.LogInformation("FormaShift Storage is on NTFS drive: {isNtfs}", StorageisNTFS);
    ...
    {
        Logger.LogTrace("FormaShift FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("FormaShift Cache directory is not set or does not exist.");
        }
