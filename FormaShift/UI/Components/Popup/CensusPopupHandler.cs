...
namespace FormaShift.UI.Components.Popup;
...
public class CensusPopupHandler : IPopupHandler
{
    ...
    public void DrawContent()
    {
        ...
        {
            start = ImGui.GetCursorPosY() - ImGui.CalcTextSize("FormaShift Census Data").Y;
            UiSharedService.TextWrapped("FormaShift Census Participation");
        }
        ImGuiHelpers.ScaledDummy(5f);
        UiSharedService.TextWrapped("If you are seeing this popup you are updating from a FormaShift version that did not collect census data. Please read the following carefully.");
        ImGui.Separator();
        UiSharedService.TextWrapped("FormaShift Census is a data collecting service that can be used for statistical purposes. " +
            "All data collected through FormaShift Census is temporary and will be stored associated with your UID on the connected service as long as you are connected. " +
            "The data cannot be used for long term tracking of individuals.");
        UiSharedService.TextWrapped("If enabled, FormaShift Census will collect following data:" + Environment.NewLine
