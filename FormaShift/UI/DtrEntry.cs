...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("FormaShift");
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"FormaShift: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "FormaShift: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "FormaShift: Not Connected";
