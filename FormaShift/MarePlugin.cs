...
public class MarePlugin : MediatorSubscriberBase, IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogInformation("Launching {name} {major}.{minor}.{build}", "Open Synchronos", version.Major, version.Minor, version.Build);
        Mediator.Publish(new EventMessage(new Services.Events.Event(nameof(MarePlugin), Services.Events.EventSeverity.Informational,
            $"Starting Open Synchronos {version.Major}.{version.Minor}.{version.Build}")));
        ...
        try
        {
            ...
            if (_mareConfigService.Current.LogLevel != LogLevel.Information)
            {
                Mediator.Publish(new NotificationMessage("Abnormal Log Level",
                    $"Your log level is set to '{_mareConfigService.Current.LogLevel}' which is not recommended for normal usage. Set it to '{LogLevel.Information}' in \"Settings -> Debug\" unless instructed otherwise.",
